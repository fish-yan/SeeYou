...
#import "HYResetSecrectViewController.h"
#import "LoginTextfiledView.h"
#import "NSString+Ext.h"
#import "HYRsetSecrectViewModel.h"
#import "RegexKit.h"
#import "SpecialRegexKit.h"
@interface HYResetSecrectViewController ()<UITextFieldDelegate>
...
@property (nonatomic ,strong) HYRsetSecrectViewModel * viewModel;
@end

@implementation HYResetSecrectViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title=self.titleString;
    self.canBack =YES;
    self.viewModel =[[HYRsetSecrectViewModel alloc] init];
    [self setUpViews];
...
-(void) setUpViews
{
    ...
    {
        proticalLablelbutton.hidden=YES;
        self.proticalLablel.hidden =YES;
...
- (UIButton *)registerButton{
    if (!_registerButton) {
        ...
        [_registerButton.titleLabel setFont: Font_PINGFANG_SC(16)];
        [_registerButton setBackgroundColor:[UIColor bt0Color]];
        [_registerButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_registerButton addTarget:self action:@selector(resetFunction:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)bindViewModel{
    ...
    if([HYUserContext shareContext].userModel.mobile.length>0)
    {
        self.phoneTextFiled.textField.text = [[HYUserContext shareContext].userModel.mobile PhoneNumberFormatString];
        self.phoneTextFiled.textField.enabled=NO;
    }
    ...
    [RACObserve(self.codeTextfiled.sendCodeButton, enabled)subscribeNext:^(NSNumber * enable) {
        @strongify(self);
        if([enable boolValue]){
            [self.codeTextfiled.sendCodeButton setTitleColor:[UIColor tc31Color] forState:UIControlStateNormal];
        }else{
    ...
    [RACObserve([HYCountDown shareHelper], resetPwdTime) subscribeNext:^(NSNumber *countdown) {
        @strongify(self);
        if ([countdown intValue] <0) {
            ...
            self.codeTextfiled.sendCodeButton.titleLabel.font =Font_PINGFANG_SC(13);
    ...
    [[self.viewModel.cutdownCommond.executionSignals switchToLatest] subscribeNext:^(id  _Nullable x) {
        ...
        @strongify(self);
        [[HYCountDown shareHelper] startWithCountDownType:WDCountDownTypeResetPwd limitedTime:59];
    ...
    [[self.viewModel.requestResetCommond.executionSignals switchToLatest]subscribeNext:^(WDResponseModel*  _Nullable x) {
        @strongify(self);
        [WDProgressHUD showTips:x.msg];
        [WDProgressHUD hiddenHUD];
        if([self.titleString isEqualToString:@"忘记密码"])
            [super popBack];
        else
        {
    ...
    [self.viewModel.requestResetCommond.errors subscribeNext:^(NSError * _Nullable x) {
         [WDProgressHUD hiddenHUD];
        [WDProgressHUD showTips:x.localizedDescription];
    }];
...
- (void)getVerycode{
    ...
    if([self.titleString isEqualToString:@"忘记密码"])
    {
      self.viewModel.codeType = VerifyCodeTypeFindPassword;
    }
    else
    {
        self.viewModel.codeType = VerifyCodeTypeResetPassword;
    }
...
- (void)keyboardDidShow:(NSNotification *)notification
{
    ...
    if(IS_IPHONE_5 || IS_IPHONE_4_OR_LESS){
        [self.scrolleView setContentOffset:CGPointMake(0, 120.) animated:YES];
    }else{
...
-(void)submitPassword
{
    [self.viewModel.requestResetCommond execute:@1];
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if([HYCountDown shareHelper].resetPwdTime >0 ){
        [self.codeTextfiled.sendCodeButton  removeAllAnimation];
        [self.codeTextfiled.sendCodeButton  loadingAnimation];
    }
